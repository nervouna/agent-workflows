...
Authorization to merge includes resolving merge conflicts autonomously within the accepted scope. A conflict alone is not a reason to ask for confirmation. Ask only when investigation cannot establish a safe resolution or an unresolved material choice requires human judgment; honor any explicit user approval gate.
...
4. If the provisional merge returns nonzero, inspect its output and `git -C INTEGRATION_WORKTREE ls-files -u`. When the failure is an ordinary merge conflict, verify the pinned worktree identity, detached `HEAD=BASE_SHA`, `MERGE_HEAD=CANDIDATE_SHA`, unchanged refs and ownership, then follow Autonomous Conflict Resolution below. For other failures, use the restoration procedure and stop as `BLOCKED`; do not add `--allow-unrelated-histories` implicitly. After a clean merge or conflict resolution, require `HEAD=BASE_SHA`, `MERGE_HEAD=CANDIDATE_SHA`, no unmerged entries, and no unstaged tracked changes; record the prospective tree with `git -C INTEGRATION_WORKTREE write-tree` plus the in-progress merge status.
...
11. On `NO-GO`, substantiate each finding. For defects confined to conflict resolution, repair the prospective merge in place under Autonomous Conflict Resolution, rerun affected checks, record the new tree and review-state snapshot, then repeat steps 5-10 with a fresh reviewer. If candidate changes are needed, restore the integration worktree, remediate and commit only those changes on the candidate, then begin a new round with new SHAs and reconstruct the merge, including corrected conflict resolutions. Never reuse a verdict after changing the reviewed tree.
...
Whenever canceling a provisional merge, first re-verify the pinned worktree identity. If `MERGE_HEAD` exists, require `git -C INTEGRATION_WORKTREE merge --abort` to succeed. If `MERGE_HEAD` is already absent, require clean tracked/untracked state and check exact-tree ignored and directory/file collisions before returning to the pinned base tree with `git -C INTEGRATION_WORKTREE switch --detach --no-overwrite-ignore BASE_SHA`; otherwise remain detached and stop. Stay detached while verifying `HEAD=BASE_SHA`, index tree `BASE_SHA^{tree}`, absent `MERGE_HEAD`, clean status, preserved ignored paths, unchanged refs and upstream snapshot, and unchanged worktree ownership. Only then may `git -C INTEGRATION_WORKTREE switch --no-overwrite-ignore BASE_BRANCH` reattach the base, followed by the same checks including symbolic `HEAD=BASE_FULL_REF`. If a ref or ownership drifted, remain detached and stop. Apart from the recoverable conflict path in step 4 and failure handling explicitly described in this skill, treat a nonzero Git mutation or identity verification result as an incomplete transaction: preserve the state, stop as `BLOCKED`, and perform no further Git mutation. If restoration itself fails, stop immediately rather than attempting destructive cleanup.

## Autonomous Conflict Resolution

1. Inspect the common ancestor versions, both branches' changes and intent, surrounding code, tests, and accepted requirements. Resolve conflicts directly in the detached provisional merge without requesting permission for routine technical decisions. Preserve both branches' intended behavior where compatible; adapt overlapping edits and regenerate derived files with the repository's normal tools when appropriate. Conflict size, file type, or overlapping edits alone do not require human input. Do not blindly select an entire side or discard behavior merely to make the merge succeed.
2. Keep changes limited to integration and necessary regression coverage. Stage exact resolved paths, including intentional deletions, and verify no unmerged entries or accidental conflict markers remain. Run affected checks against the resolved tree; all repository-required checks and independent `GO` review still apply before integration. Do not commit the provisional merge or advance either branch before the final merge gate.
3. If evidence and validation cannot establish a safe resolution, or the branches encode incompatible requirements with no authorized precedence, record the affected paths, competing behaviors, evidence, and smallest decision needed. Use the restoration procedure before asking one focused question; report `BLOCKED` and any restoration failure. Do not ask merely because a conflict exists, and do not repeat a choice the user has already made.

Pass the originally conflicted paths as raw facts to the reviewer so the resolved interactions receive explicit scrutiny, without supplying a preferred verdict. Retain a concise account of resolutions and their validation for the final report.
...
- conflicts resolved autonomously, resolution rationale and validation, or the specific unresolved human decision
